...
function scene:show( event )
  ...
  if phase == "will" then
    ...
    testerButton = display.newText("Unit Tests", 460, 10, native.systemFont, 20)

    sceneGroup:insert(levelSelectionButton)
    sceneGroup:insert(settingsButton)
    sceneGroup:insert(creditsButton)
    sceneGroup:insert(exitButton)
    sceneGroup:insert(testerButton)
  elseif phase == "did" then
    ...
    if testerButton then
      function testerButton:touch ( event )
        local phase = event.phase
        if "ended" == phase then
          composer.gotoScene( "scenes.testerScene", { effect = "fade", time = 300 } )
        end
      end
